...
- G2: 6블록, TODO 0건, 본문 통계성 수치 출처율 60% 이상, 원어 병기, 출제포인트/혼동주의
...
저장소에 포함된 샘플 `3-1`은 새 체크아웃에서 바로 실행할 수 있도록 `pending` 상태로 제공한다. 파이프라인 실행 결과는 `outputs/`에 생성되며 저장소에는 포함되지 않는다.
...
- `inputs/sections/3-1.md`는 실행 형식을 보여주기 위해 새로 작성한 가상 샘플이며 실제 교재에서 추출한 문장이 아니다.
